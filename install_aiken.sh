...
APP_NAME="aikup"
APP_VERSION="0.0.11"
ARTIFACT_DOWNLOAD_URL="${INSTALLER_DOWNLOAD_URL:-https://github.com/aiken-lang/aikup/releases/download/v0.0.11}"
PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"install_prefix":"AXO_INSTALL_PREFIX","provider":{"source":"cargo-dist","version":"0.14.1"},"source":{"app_name":"aikup","name":"aikup","owner":"aiken-lang","release_type":"github"},"version":"0.0.11"}
EORECEIPT
# Are we happy with this same path on Linux and Mac?
RECEIPT_HOME="${HOME}/.config/aikup"
...
The installer for aikup 0.0.11

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/aiken-lang/aikup/releases/download/v0.0.11
...
download_binary_and_run_installer() {
    ...
    case "$_arch" in
        "aarch64-apple-darwin")
            _artifact_name="aikup-aarch64-apple-darwin.tar.gz"
            _zip_ext=".tar.gz"
            _bins="aikup"
            _bins_js_array='"aikup"'
            _updater_name=""
            _updater_bin=""
            ;;
        "x86_64-apple-darwin")
            _artifact_name="aikup-x86_64-apple-darwin.tar.gz"
            _zip_ext=".tar.gz"
            _bins="aikup"
            _bins_js_array='"aikup"'
            _updater_name=""
        ...
        "x86_64-pc-windows-gnu")
            _artifact_name="aikup-x86_64-pc-windows-msvc.tar.gz"
            _zip_ext=".tar.gz"
            _bins="aikup.exe"
            _bins_js_array='"aikup.exe"'
            _updater_name=""
            _updater_bin=""
            ;;
        "x86_64-unknown-linux-gnu")
            _artifact_name="aikup-x86_64-unknown-linux-gnu.tar.gz"
            _zip_ext=".tar.gz"
            _bins="aikup"
            _bins_js_array='"aikup"'
            _updater_name=""
...
install() {
    ...
    if [ -z "${_install_dir:-}" ]; then
        # Install to $HOME/.aiken/bin
        if [ -n "${HOME:-}" ]; then
            _install_dir="$HOME/.aiken/bin"
            _env_script_path="$HOME/.aiken/bin/env"
            _install_dir_expr='$HOME/.aiken/bin'
            _env_script_path_expr='$HOME/.aiken/bin/env'
        fi
    fi
